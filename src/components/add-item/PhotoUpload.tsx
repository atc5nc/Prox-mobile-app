...
export function PhotoUpload({ onBack, onSuccess }: PhotoUploadProps) {
  ...
  const processImage = async (file: File) => {
    ...
    try {
      ...
      // Mock parsed items - replace with actual OCR results
      const mockItems: ParsedItem[] = [
        { name: 'Organic Bananas', category: 'Produce', confirmed: true },
        { name: 'Whole Milk', category: 'Dairy', confirmed: true },
        { name: 'Ground Turkey', category: 'Meat', confirmed: true },
        { name: 'Sourdough Bread', category: 'Pantry', confirmed: true },
      ];
      ...
      toast({
        ...
        description: `Found ${mockItems.length} items. Review and confirm below.`,
      });
    } catch (error) {
      toast({
